...
export async function fetchAndLoadPages(initialPageNumber = 1) {
  ...
  try {
    ...
    if (!state.currentChapter?.groups?.LesPoroïniens) {
      throw new Error("Ce chapitre n'a pas de données ImgChest (probablement un LN).");
    }
    const chapterId = state.currentChapter.groups.LesPoroïniens.split("/").pop(); 
